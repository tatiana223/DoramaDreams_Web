        ...
        <div>
          {!selectedDorama ? (
            ...
            <>
              ...
              <div className="mt-4 rounded-[1.5rem] border border-violet-200/80 bg-violet-50/70 p-4 dark:border-violet-400/20 dark:bg-violet-500/10">
                <div className="flex items-start gap-3">
                  <div className="mt-1 rounded-2xl bg-violet-600 p-2 text-white">
                    <UploadCloud className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-sm font-black">Массовое добавление</p>
                    <p className="mt-1 text-xs font-semibold leading-5 text-muted-foreground dark:text-white/55">
                      Много серий сразу — это удобно 💜 <span className="font-black">номер | ссылка</span>.
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 grid gap-4 xl:grid-cols-[160px_minmax(0,1fr)]">
                  <label className="grid min-w-0 gap-2">
                    <span className="text-sm font-black">Начать с серии</span>
                    <input
                      type="number"
                      min={1}
                      value={bulkStartNumber}
                      onChange={(event) => setBulkStartNumber(event.target.value)}
                      className="w-full rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm font-semibold outline-none focus:border-violet-500 dark:border-white/10 dark:bg-white/10"
                    />
                  </label>

                  <label className="flex min-w-0 items-center gap-3 rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm font-bold leading-5 dark:border-white/10 dark:bg-white/10">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="min-w-0">Перезаписывать серии, если такой номер уже есть</span>
                  </label>
                </div>
                ...
                <textarea
                  ...
                  placeholder={`<iframe src="https://vkvideo.ru/video_ext.php?..." ...></iframe>\n<iframe src="https://vkvideo.ru/video_ext.php?..." ...></iframe>\n3 | https://vkvideo.ru/video_ext.php?...`}
                  className="mt-3 min-h-[190px] w-full rounded-2xl border border-border/70 bg-background/80 px-4 py-3 text-sm font-semibold outline-none focus:border-violet-500 dark:border-white/10 dark:bg-white/10"
                />

                <button
                  onClick={handleBulkImport}
                  disabled={isBulkSaving}
                  className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-violet-600 px-5 py-3 text-sm font-black text-white transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isBulkSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <UploadCloud className="h-4 w-4" />}
