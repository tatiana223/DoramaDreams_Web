...
export function AdminActorManager() {
  ...
  const filteredActors = useMemo(() => {
    ...
    return actors
      .filter((actor) => {
        ...
        const biography = actor.biography?.toLowerCase() ?? "";
        return name.includes(normalized) || biography.includes(normalized);
      })
      .slice(0, 18);
  }, [actors, actorQuery]);
  ...
  const filledActorsCount = useMemo(() => {
    return actors.filter((actor) => actor.photoUrl || actor.biography).length;
  }, [actors]);
  ...
  return (
    <section className="overflow-hidden rounded-[2rem] border border-border/70 bg-card/85 shadow-xl shadow-violet-950/10 backdrop-blur-xl dark:border-white/10 dark:bg-white/[0.06] dark:shadow-black/20">
      <div className="relative border-b border-border/70 p-6 dark:border-white/10 md:p-8">
        ...
        <div className="relative flex flex-col justify-between gap-6 xl:flex-row xl:items-end">
          <div className="max-w-3xl">
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground dark:text-white/55">
              Здесь можно создать актёра, заполнить фото и биографию, а затем привязать его к нужной дораме для блока «В ролях».
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-3 xl:min-w-[520px]">
            <ActorStat value={actors.length} label="актёров" />
            <ActorStat value={filledActorsCount} label="с фото/био" />
      ...
      {isLoading ? (
        ...
        <div className="grid gap-6 p-6 md:p-8 xl:grid-cols-[0.92fr_1.08fr]">
          <div className="space-y-6">
            <div className="rounded-[1.6rem] border border-border/70 bg-background/55 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="text-xl font-black">Карточка актёра</h3>
                  <p className="mt-1 text-sm text-muted-foreground dark:text-white/50">
                    Добавляй имена так, как они должны отображаться на странице дорамы.
                  </p>
                </div>
                {editingActor && (
                  <button
              ...
              <div className="mt-5 grid gap-3">
                <label className="grid gap-2">
                  <span className="text-sm font-black">ФИО / имя актёра</span>
                  <input
                    value={form.fullName}
                    onChange={(event) => updateForm("fullName", event.target.value)}
                    placeholder="Например: Ли Мин Хо"
                    className="rounded-2xl border border-border/70 bg-card/80 px-4 py-3 text-sm font-semibold outline-none transition focus:border-violet-500 dark:border-white/10 dark:bg-black/20"
                  />
                </label>
                ...
                <label className="grid gap-2">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="resize-none rounded-2xl border border-border/70 bg-card/80 px-4 py-3 text-sm font-semibold leading-6 outline-none transition focus:border-violet-500 dark:border-white/10 dark:bg-black/20"
                  />
                </label>
              </div>
            ...
            <div className="rounded-[1.6rem] border border-border/70 bg-background/55 p-5 dark:border-white/10 dark:bg-white/5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted-foreground dark:text-white/50">Поиск, редактирование и удаление.</p>
                </div>
              </div>
                  ...
                  <div className="rounded-[1.5rem] border border-border/70 bg-card/70 p-4 dark:border-white/10 dark:bg-black/15">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        ...
                        <h4 className="mt-2 text-lg font-black leading-tight">{selectedDorama.title}</h4>
                        <p className="mt-1 text-xs text-muted-foreground dark:text-white/45">{selectedDorama.originalTitle || "Оригинальное название не указано"}</p>
                      </div>
                    </div>
                        ...
                        <div className="mt-3 flex flex-wrap gap-2">
                          {selectedActors.map((actor) => (
                            <button
                              ...
                              className="inline-flex items-center gap-2 rounded-full bg-violet-600 px-3 py-1.5 text-xs font-black text-white transition hover:bg-violet-700"
                            >
                              {actor.fullName}
                      ...
                      actors.map((actor) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              <p className="truncate font-black">{actor.fullName}</p>
                              <p className="truncate text-xs text-muted-foreground dark:text-white/45">{actor.biography || "Без описания"}</p>
                            </div>
                          </button>
                        );
                      })
...
function ActorPreview({ form }: { form: ActorForm }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-2xl border border-border/70 bg-card/70 p-3 dark:border-white/10 dark:bg-black/15">
      {form.photoUrl.trim() ? (
        <img src={form.photoUrl.trim()} alt={name} className="h-12 w-12 rounded-full object-cover" />
      ) : (
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-violet-100 text-violet-700 dark:bg-white/10 dark:text-violet-200">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-xs text-muted-foreground dark:text-white/45">
          {form.biography.trim() || "Предпросмотр карточки"}
        </p>
      </div>
    </div>
  );
