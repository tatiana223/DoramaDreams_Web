...
import type { AdminDashboard, AdminDoramaPayload, RecommendationImportResponse } from "@/types/admin";
...
export async function generateMockCatalog(): Promise<string> {
  const response = await fetch("/api/admin/test-data/catalog", {
    method: "POST",
    headers: authHeaders(),
  });
  ...
  if (!response.ok) {
    throw new Error(text || "Не удалось сгенерировать мок-каталог");
  }

  return text;
}

export async function generateMockUsers(users: number): Promise<string> {
  const searchParams = new URLSearchParams({ users: String(users) });

  const response = await fetch(`/api/admin/test-data/generate?${searchParams.toString()}`, {
    method: "POST",
    headers: authHeaders(),
  });
  ...
  if (!response.ok) {
    throw new Error(text || "Не удалось сгенерировать мок-пользователей");
  }

  return text;
}

export async function generateAllMockData(users: number): Promise<string> {
  const searchParams = new URLSearchParams({ users: String(users) });

  const response = await fetch(`/api/admin/test-data/all?${searchParams.toString()}`, {
    method: "POST",
    headers: authHeaders(),
  });
  ...
  if (!response.ok) {
    throw new Error(text || "Не удалось сгенерировать мок-данные");
  }
